...
contract TollToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 TOLLs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to Toll's router when our own AMM release
    IUniswapV2Router02 public tollRouter;
    // The trading pair
    address public tollPair;
    // In swap and liquify
    ...
    event TollRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "TOLL::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the TOLL Token contract.
     */
    constructor() public BEP20("Toll_V2", "TOLL") {
    ...
    /// @dev overrides transfer function to meet tokenomics of TOLL
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "TOLL::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "TOLL::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the toll pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = tollRouter.WETH();

        _approve(address(this), address(tollRouter), tokenAmount);

        // make the swap
        tollRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        tollRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from tollRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "TOLL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "TOLL::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "TOLL::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateTollRouter(address _router) public onlyOperator {
        tollRouter = IUniswapV2Router02(_router);
        tollPair = IUniswapV2Factory(tollRouter.factory()).getPair(address(this), tollRouter.WETH());
        require(tollPair != address(0), "TOLL::updateTollRouter: Invalid pair address.");
        emit TollRouterUpdated(msg.sender, address(tollRouter), tollPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "TOLL::transferOperator: new operator is the zero address");
