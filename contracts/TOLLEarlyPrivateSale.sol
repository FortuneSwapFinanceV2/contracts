...
contract TOLLEarlyPrivateSale {
    ...
    uint256 public totalTokensToSell = 15000000 * 10**18;          // 15000000 TOLL tokens for sell
    uint256 public tollPerBnb = 15000 * 10**18;             // 1 BNB = 15000 TOLL
    ...
    mapping(address => uint256) public tollPerAddresses;
    ...
    // Function to buy TOLL using BNB token
    function buyWithBNB(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = tollPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        tollPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        TOLL.transfer(msg.sender, buyAmount);
    ...
    // function to set the total tokens to sell
    // only owner can call this function
    function setTokenPricePerBNB(uint256 _tollPerBnb) public {
        require(msg.sender == owner);
        require(_tollPerBnb > 0, "Invalid TOLL price per BNB");
        tollPerBnb = _tollPerBnb;
    }
    ...
    function withdrawUnsoldTokens() public {
        ...
        TOLL.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return TOLL.balanceOf(address(this));
    }

    //function to calculate the quantity of TOLL token based on the TOLL price of bnbAmount
    function calculateTOLLAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 tollAmount = tollPerBnb.mul(bnbAmount).div(10**18);
        return tollAmount;
    }

    //function to calculate the quantity of bnb needed using its TOLL price to buy `buyAmount` of TOLL tokens.
    function calculateBNBAmount(uint256 tollAmount) public view returns (uint256) {
        require(tollPerBnb > 0, "TOLL price per BNB should be greater than 0");
        uint256 bnbAmount = tollAmount.mul(10**18).div(tollPerBnb);
        return bnbAmount;
    }
}